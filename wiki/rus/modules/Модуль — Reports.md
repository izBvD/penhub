...
| **🔑 REUSED PASSWORDS ↓** | Все логины с одинаковыми паролями. Одна строка на секрет (пароль/хеш), использованный в ≥2 местах: перечисление всех `domain\login` (учитываются LSA+SAM, custom и все источники) + DPAPI `url;login` с тем же паролем + число повторений. XLSX. Может быть использовано для поиска фактов регистрации на сторонних сайтах с тем же корпоративным паролем. |
