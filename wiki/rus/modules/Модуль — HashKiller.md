...
**➕ ADD BY PASSWORD** (подблок) — вставьте или загрузите список паролей в открытом виде, по одному на строку. Каждый пароль локально хэшируется в NT и пишется парой `hash:plaintext` — быстрый способ закинуть словарь или пачку известных паролей прямо в базу, минуя hashcat. Пустые строки пропускаются.
